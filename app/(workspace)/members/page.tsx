...
const openableRoles = [Role.DEPUTY_SECRETARY, Role.MINISTER, Role.VICE_MINISTER, Role.MEMBER, Role.GUEST];
  ...
  return (
    <div className="p-6 lg:p-10 max-w-[1000px] mx-auto">
      <motion.div initial="hidden" animate="show" variants={{ show: { transition: { staggerChildren: 0.05 } } }}>
        ...
        {creating && <motion.form variants={fadeUp} onSubmit={createMember} className="mb-6 grid gap-3 rounded-xl border border-border bg-white p-4 sm:grid-cols-2"><input required value={newMember.name} onChange={(e) => setNewMember({ ...newMember, name: e.target.value })} placeholder="姓名" className="h-10 rounded-lg border px-3 text-sm" /><input required type="email" value={newMember.email} onChange={(e) => setNewMember({ ...newMember, email: e.target.value })} placeholder="name@stu.njnu.edu.cn" className="h-10 rounded-lg border px-3 text-sm" /><select value={newMember.role} onChange={(e) => setNewMember({ ...newMember, role: e.target.value as Role })} className="h-10 rounded-lg border px-3 text-sm">{openableRoles.map((role) => <option key={role} value={role}>{RoleLabel[role]}</option>)}</select><select value={newMember.departmentId} onChange={(e) => setNewMember({ ...newMember, departmentId: e.target.value })} className="h-10 rounded-lg border px-3 text-sm"><option value="">不分配部门</option>{departments.map((department) => <option key={department.id} value={department.id}>{department.name}</option>)}</select><div className="sm:col-span-2 flex items-center justify-between"><span className="text-xs text-muted-foreground">新账号将使用管理员配置的初始密码，并在首次登录时强制修改。</span><button className="h-9 rounded-lg bg-foreground px-4 text-sm text-background">开通账号</button></div></motion.form>}
