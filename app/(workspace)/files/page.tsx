...
export default function FilesPage() {
  ...
  const [moveTarget, setMoveTarget] = useState<ApiFolder | null>(null);
  ...
  const moveFolder = async () => { if (!moveTarget) return; const response = await fetch(`/api/folders/${moveTarget.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ parentId: moveParentId || null }) }); const payload = await response.json() as ApiResponse<unknown>; if (!response.ok || !payload.success) toast.error(payload.error?.message ?? '移动失败。'); else { toast.success('文件夹已移动。'); setMoveTarget(null); setMoveParentId(''); void fetch('/api/folders', { cache: 'no-store' }).then(r=>r.json()).then((p:ApiResponse<ApiFolder[]>)=>{if(p.success)setFolders(p.data??[]);}); } };
