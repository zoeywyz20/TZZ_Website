...
export async function GET() {
  try {
    ...
    const canViewAcrossDepartments = [AppRole.SUPER_ADMIN, AppRole.SECRETARY].includes(user.role);
    const members = await getDb().profile.findMany({ where: canViewAcrossDepartments ? undefined : { departmentId: user.departmentId ?? '__no_department__' }, select: { id: true, name: true, email: true, avatar: true, role: true, departmentId: true, accountEnabled: true, mustChangePassword: true, department: { select: { id: true, name: true, shortName: true } } }, orderBy: { joinedAt: 'asc' } });
