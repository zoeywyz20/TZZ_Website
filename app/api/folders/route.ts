...
export async function GET(request: Request) {
  ...
  } catch (error) { if (error instanceof AuthenticationError) return apiError('UNAUTHORIZED', error.message, 401); if (error instanceof Error && error.message === 'FOLDER_FORBIDDEN') return apiError('FORBIDDEN', '无权浏览目录。', 403); return apiError('REQUEST_FAILED', '目录读取失败。', 500); }
}

export async function POST(request: Request) {
  try { const user = await requireUser(); const data = schema.safeParse(await request.json().catch(() => null)); if (!data.success) return apiError('VALIDATION_ERROR', '目录数据不合法。', 400); return apiSuccess(await createFolder(user, data.data), 201); }
  catch (error) { if (error instanceof AuthenticationError) return apiError('UNAUTHORIZED', error.message, 401); return folderError(error); }
}
