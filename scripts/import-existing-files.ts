...
import { ImportStatus, PrismaClient, type Folder } from '../generated/prisma/client';
import { createStorageKey, storageRoots } from '../lib/storage-core';

type Options = { dryRun: boolean; uploaderId?: string; departmentId?: string; source?: string };

async function main() {
  const options = parseArgs(process.argv.slice(2));
  if (!options.uploaderId || !options.source) throw new Error('Usage: tsx scripts/import-existing-files.ts --source <stable-source-id> --uploader-id <profile-uuid> [--department-id <uuid>] [--dry-run]');
  ...
  try {
    ...
    for (const source of await filesUnder(importRoot)) {
      ...
      if (options.dryRun) { console.log(`DRY-RUN ${relativePath} -> ${folderPath(relativePath).join('/') || '(root)'}`); continue; }
      ...
      const file = existingFile ?? await db.$transaction(async (tx) => {
        const created = await tx.fileRecord.create({ data: { originalFilename: path.basename(source), storageKey: pending.storageKey, mimeType: mimeType(source), size: BigInt(info.size), hash, uploaderId: uploader.id, departmentId, folderId } });
